...
| `applyScroll` pins `stage.scrollLeft` to 0 | `overflow: hidden` stops a finger, not `scrollIntoView()`. Once the strip may be wider than the stage, an arrow key or a click on a half-visible row scrolls the stage itself and drags every column out through the left edge — the dial writes `#finder`, so nothing would ever put them back |
