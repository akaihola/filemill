...
```python src/filemill/rendering.py
...
can rewrite it as a filemill URL. The search strategy is:
...
to filemill URLs so clicking them navigates within the app instead of
hitting a 404.
...
```python src/filemill/preview.py
import configparser
import html as html_lib
from pathlib import Path
from urllib.parse import quote as urlquote
...
uv sync
python -c "
from filemill.rendering import md
