...
| Path                                        | Purpose                                                                                                 |
| ------------------------------------------- | ------------------------------------------------------------------------------------------------------- |
| `src/filemill/vfs.py`                     | `VFSEntry`, `VFSProvider` protocol, `VFSRegistry`, `REGISTRY` singleton, `is_vfs_file()`, `_truncate()` |
| `src/filemill/providers/__init__.py`      | Empty package marker                                                                                    |
| `src/filemill/providers/sqlite.py`        | `SQLiteProvider` – schemas/tables/rows + preview                                                        |
| `src/filemill/providers/csv_provider.py`  | `CSVProvider` stub                                                                                      |
| `src/filemill/providers/json_provider.py` | `JSONProvider` stub                                                                                     |

### Modified modules

| Module                      | Changes                                                                                                                                                                       |
| --------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `src/filemill/columns.py` | `list_vfs_column()` (new); `entry_icon()` adds `.db → 🗄️`; `list_column()` treats VFS files like directories (targets next col); `render_breadcrumb()` gains `vpath=""` param |
| `src/filemill/app.py`     | `/click` gains `vpath: str = ""`, `fmt: str = ""`; VFS dispatch block; `_make_bc_oob()` + `_build_prune_js()` helpers extracted; `/vpage` new route                           |
| `src/filemill/styles.py`  | CSS and JS additions                                                                                                                                                          |
...
> when the provider does `from filemill.vfs import VFSEntry` mid-execution, it
...
### 3.2 `src/filemill/providers/__init__.py`

Empty file.

---

### 3.3 `src/filemill/providers/sqlite.py`
...
from filemill.vfs import VFSEntry, REGISTRY, _truncate
...
### 3.4 `src/filemill/providers/csv_provider.py`

```python
from __future__ import annotations
from pathlib import Path
from filemill.vfs import VFSEntry, REGISTRY
...
### 3.5 `src/filemill/providers/json_provider.py`
...
### 3.6 `src/filemill/columns.py` – changes
...
### 3.7 `src/filemill/app.py` – changes

#### Imports to add

```python
from filemill.vfs import REGISTRY
from filemill.columns import list_vfs_column
```
...
### 3.8 `src/filemill/styles.py` – changes
...
### Step 3 – Green: `src/filemill/vfs.py`
...
from pathlib import Path
from filemill.providers.sqlite import SQLiteProvider, TABLE_ICON, SCHEMA_ICON, ROW_ICON
...
Create `src/filemill/providers/__init__.py` (empty).

Create `src/filemill/providers/sqlite.py` with the complete structure from §3.3,
...
Create `src/filemill/providers/csv_provider.py` and
`src/filemill/providers/json_provider.py` exactly as in §3.4 and §3.5.
...
#   from filemill.columns import render_breadcrumb
...
2. Add `from filemill.vfs import is_vfs_file` import.
...
1. Add imports: `from filemill.vfs import REGISTRY`, `from filemill.columns import
...
- `src/filemill/vfs.py` – `VFSEntry`, `VFSProvider` (Protocol), `VFSRegistry`,
  `REGISTRY` singleton, `is_vfs_file()`, `_truncate()`
- `src/filemill/providers/__init__.py` – package marker
- `src/filemill/providers/sqlite.py` – `SQLiteProvider` with schema/table/row
  enumeration, row-key derivation (PK → unique col → rowid), spreadsheet preview
  (paginated), KV row-detail preview, BLOB handling
- `src/filemill/providers/csv_provider.py` – stub (`default_fmt="spreadsheet"`)
- `src/filemill/providers/json_provider.py` – stub (`default_fmt="formatted"`)
- `src/filemill/columns.py` – `list_vfs_column()`, `🗄️` icon for `.db`,
  VFS-file routing in `list_column()`, `vpath` param in `render_breadcrumb()`
- `src/filemill/app.py` – `/click` VFS dispatch (`vpath`, `fmt` params),
  `_make_bc_oob()`, `_build_prune_js()` helpers, `/vpage` endpoint
- `src/filemill/styles.py` – VFS/DB CSS, format-persistence JS (localStorage)
