import {
  ...
  nextRow,
  pageSelection,
} from "./model/keyboard.js";
import { autoPreview, selectNode } from "./model/selection.js";
/* ═══════════════════════════════════════════════════════════════════════════
   Navigation
...
import { finder } from "./dom-renderer.js";
import { taCancel, taLive, taType } from "./typeahead.js";
...
function pageMove(c, dir, ci) {
  ...
  const visible = () => {
    ...
    return c.rows.flatMap((row, index) => {
      if (!row) return [];
      const rect = row.getBoundingClientRect();
      return rect.bottom > bounds.top && rect.top < bounds.bottom
        ? [{ index, height: row.offsetHeight }]
        : [];
    });
  };
  let decision = pageSelection(visible(), ci, dir, body.clientHeight);
  if (decision.scroll) {
    body.scrollTop += decision.scroll;
    decision = pageSelection(visible(), undefined, dir, body.clientHeight);
  }
  const row = c.rows[decision.index];
  row?.click();
...
document.addEventListener("keydown", (e) => {
  const context = {
    editable: e.target.matches?.("input, textarea, select") ||
      e.target.isContentEditable,
    welcome: !document.getElementById("welcome")?.hidden,
    preview: !!e.target.closest?.("#preview"),
    textSelected: !!getSelection().toString(),
    typeahead: taLive(),
  };
  const action = keyAction(e, context);
  if (context.editable || context.welcome) return;
  if (context.preview) {
    if (action === "preview-back") {
      e.preventDefault();
      unfoldTo(focusCol);
      render(true);
      const parent = colCache.get(path[focusCol]);
      const ri = rowIndex(path[focusCol], sel[focusCol]);
      if (parent && ri >= 0) revealRow(parent.ensureRow(ri));
      return;
    }
    if (action === "preview-scroll") {
  ...
  } else if (action === "ArrowRight" || action === "Enter") {
    e.preventDefault();
