...
Adapt Filemill’s FastHTML/ASGI app for embedding in the gogo dashboard while preserving standalone behavior. This is a plan only; no production implementation belongs in this change.
...
- `src/filemill/app.py`: `/click`, `/raw`, `/w/{path:path}`, `/f/`, `/f/{path:path}`, `_resolve_safe()`, mount URL helpers, shell/deep-navigation JavaScript.
- `src/filemill/columns.py`: Finder links, HTMX targets, `data-finder-url`, breadcrumb.
- `src/filemill/preview.py`: Markdown dispatch, Pygments code dispatch, UTF-8 raw fallback, PDF/image `/raw` URLs.
- `src/filemill/rendering.py`: Markdown links/wikilinks and current `/f/`, `/w/`, or `/raw?path=` URL generation.
...
- `/docs/readme.md?filemill-view=rendered` serves the rendered view.
- `/docs/readme.md?filemill-view=highlighted` serves the syntax-highlighted view.
- `/docs/readme.md?filemill-view=raw` serves the raw view.
...
- The same path with `filemill-view=rendered`, `highlighted`, or `raw` selects
  the matching representation.
- Directory paths render the selected column layout when the Filemill view is
  requested.
...
- Preserve `filemill-view`, `layout`, and `hidden` when the dashboard router
...
- Markdown uses `filemill-view=rendered`; source/code uses
  `filemill-view=highlighted`. `filemill-view=raw` returns the source.
...
- Rendered Markdown links keep the same relative URL path and add the required
  `filemill-view` value. Non-Markdown assets use the same path with the raw
...
- A visible switch link/button has a stable class or data attribute and links to
  the same path with `filemill-view=rendered`, `highlighted`, or `raw`.
...
- Define deterministic behavior for absent, empty, repeated, malformed, and
  encoded route paths, `filemill-view`, `layout`, `hidden`, and any VFS
  `vpath` parameters; test it.
- Preserve valid `filemill-view`, `layout`, and `hidden` values through
...
- `src/filemill/app.py`: shared URL/query parsing/building; root-relative
  routes; static/raw/rendered mode dispatch; route ordering and query
  propagation.
- `src/filemill/columns.py`: preserve layout/hidden state in internal links
  and deep-navigation metadata without breaking HTMX targets.
- `src/filemill/preview.py`: explicit rendered/raw mode, switch markup, Markdown/code integration, reciprocal links, and existing fallbacks.
- `src/filemill/rendering.py`: pass view/query context into Markdown and wikilink hrefs; retain relative-link and mount fallback behavior.
- `src/filemill/styles.py`: only minimal switch-control CSS if existing preview-bar styles cannot be reused.
...
- Every internal generated link keeps the same root-relative path and valid
  `filemill-view`, `layout`, and `hidden` query values.
