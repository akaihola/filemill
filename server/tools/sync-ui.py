...
`src/filemill/ui/` is a copy of it and must never be hand-edited. The copy is
about packaging, not drift: a wheel cannot reach outside its own package
directory, so what `pip install filemill` needs at runtime has to live inside
...
PKG = Path(__file__).resolve().parent.parent / "src" / "filemill"
DEST = PKG / "ui"
